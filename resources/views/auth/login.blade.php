        ...
        <div class="flex flex-wrap justify-center -mx-3 -mt-48 lg:-mt-48 md:-mt-56">
            <div class="w-full max-w-full px-3 mx-auto shrink-0 md:flex-0 md:w-7/12 lg:w-5/12 xl:w-4/12">
                <div class="relative z-0 flex flex-col min-w-0 break-words bg-white border-0 dark:bg-gray-950 dark:shadow-soft-dark-xl shadow-soft-xl rounded-2xl bg-clip-border">
                    ...
                    <div class="flex flex-wrap px-4 -mx-3 sm:px-6 xl:px-12">
                    ...
                    <div class="flex-auto p-6 text-center">
                        <form role="form text-left" action="{{route('login')}}" method="post">
                            @csrf
                            <div class="mb-4">
{{--                                <input type="email" class="text-sm focus:shadow-soft-primary-outline dark:bg-gray-950 dark:placeholder:text-white/80 dark:text-white/80 leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow" placeholder="Email" aria-label="Email" aria-describedby="email-addon" />--}}
                                <x-panel.input-label for="login" :value="__('Username or Email')" class="text-left"/>
                                <x-panel.text-input id="login" class="" type="text" name="login" :value="old('login')" required autofocus autocomplete="login" placeholder="Login" />
                                <x-panel.input-error :messages="$errors->get('login')" class="mt-2" />
                            </div>
                            <div class="mb-4">
{{--                                <input type="password" class="text-sm focus:shadow-soft-primary-outline dark:bg-gray-950 dark:placeholder:text-white/80 dark:text-white/80 leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow" placeholder="Password" aria-label="Password" aria-describedby="password-addon" />--}}
                                <x-panel.input-label for="password" :value="__('Password')" class="text-left"/>

                                <x-panel.text-input id="password" class=""
                                                type="password"
                                                name="password"
                                                required autocomplete="current-password"
                                                placeholder="Password"                      />

                                <x-panel.input-error :messages="$errors->get('password')" class="mt-2" />
                            </div>
                            <div class="min-h-6 mb-0.5 block pl-12 text-left">
                                <input id="remember_me" type="checkbox" class="mt-0.5 rounded-10 duration-250 ease-soft-in-out after:rounded-circle after:shadow-soft-2xl after:duration-250 checked:after:translate-x-5.3 h-5 relative float-left -ml-12 w-10 cursor-pointer appearance-none border border-solid border-gray-200 bg-slate-800/10 bg-none bg-contain bg-left bg-no-repeat align-top transition-all after:absolute after:top-px after:h-4 after:w-4 after:translate-x-px after:bg-white after:content-[''] checked:border-slate-800/95 checked:bg-slate-800/95 checked:bg-none checked:bg-right" name="remember" checked="" />
                                <label class="mb-2 ml-1 font-normal cursor-pointer select-none text-sm text-slate-700" for="rememberMe">{{__('Remember
                                    me')}}</label>
                            </div>
                            ...
                            <div class="text-center">
                                <a href="{{route('register')}}" class="inline-block w-full px-6 py-3 mt-2 mb-6 font-bold text-center text-white uppercase align-middle transition-all bg-transparent border-0 rounded-lg cursor-pointer active:opacity-85 hover:scale-102 hover:shadow-soft-xs leading-pro text-xs ease-soft-in tracking-tight-soft shadow-soft-md bg-150 bg-x-25 bg-gradient-to-tl from-gray-900 to-slate-800 hover:border-slate-700 hover:bg-slate-700 hover:text-white">{{__('Sign
                                    up')}}</a>
                            </div>
                            @if (Route::has('password.request'))
                            <div class="text-center">
                                <a class="inline-block w-full px-6 py-3 mt-2 mb-6 font-bold text-center text-white uppercase align-middle transition-all bg-transparent border-0 rounded-lg cursor-pointer active:opacity-85 hover:scale-102 hover:shadow-soft-xs leading-pro text-xs ease-soft-in tracking-tight-soft shadow-soft-md bg-150 bg-x-25 bg-gradient-to-tl from-gray-900 to-slate-800 hover:border-slate-700 hover:bg-slate-700 hover:text-white" href="{{ route('password.request') }}">
...
{{--        <!-- Email Address -->--}}
{{--        <div class="mb-4">--}}
{{--            <x-panel.input-label for="login" :value="__('Username or Email')" />--}}
{{--            <x-panel.text-input id="login" class="" type="text" name="login" :value="old('login')" required autofocus autocomplete="login" placeholder="Login" />--}}
{{--            <x-panel.input-error :messages="$errors->get('login')" class="mt-2" />--}}
{{--        </div>--}}
...
{{--        <!-- Password -->--}}
{{--        <div class="mb-4">--}}
{{--            <x-panel.input-label for="password" :value="__('Password')" />--}}
...
{{--            <x-panel.primary-button class="ml-3">--}}
{{--                {{ __('Log in') }}--}}
